...
/* enums */
enum { CurResizeBR, CurResizeBL, CurResizeTR, CurResizeTL, CurResizeHorzArrow, CurResizeVertArrow, CurNormal, CurMove, CurLast }; /* cursor */
...
static void resizeormfact(const Arg *arg);
...
focusmaster(const Arg *arg)
{
	...
	if (selmon->sel == master) {
		if (selmon->tagmarked[i] && ISVISIBLE(selmon->tagmarked[i])) {
			...
			// XRaiseWindow(dpy, selmon->tagmarked[i]->win);
			restack(selmon);
		}
	} else {
...
tilewide(Monitor *m)
{
        unsigned int i, n, w, h, mw, mx, my, ty;
	float mfacts = 0, sfacts = 0;
	Client *c;
	...
	if (n > m->nmaster)
		...
		mw = m->ww;
	for (i = mx = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			h = (m->wh - my) * (c->cfact / mfacts);
